...
public static void capacite(int id){
	if (id<6){
	...
	if (id==8){
		...
		while (choix){
			...
			Divine=sc.nextInt();
			while (it.hasNext()){
				if (it.next().getId()==Divine && !it.next().equals(Partie.joueurEncours)&& it.next().laMain.listeCarteA.size()>2){
	...
	if (id==11){}
	if (id==12){
		//guide revient dans sa main et croyant lie revient au centre de la table
	}
	if (id==13){
	...
	if (id==25){
		...
		while (choix){
			...
			if(Divine>Partie.joueurEncours.getId() && Divine < Partie.listeJoueurs.size()){
				...
				Partie.joueurEncours.ptAction_Neant=Partie.joueurEncours.ptAction_Neant+Partie.listeJoueurs.get(Divine-1).ptAction_Neant;
	...
	if (id==26){
		//benificier une capacite speciale dune carte croyant appartenant a un autre joueur 
	}
	if (id>26 && id<32){
